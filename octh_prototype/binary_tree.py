...
class BinClassNode:
    ...
    def __init__(self, is_leaf, applies_split=None, split_on=None, split_value=None, label=None):
    ...
    def __str__(self):
        if self.is_leaf:
            ...
            return 'Branch node\napplies a split: {0}\nsplits on: {1}\nsplit value: {2}\n*****'.format(
                self.applies_split, self.split_on, self.split_value)
...
class BinClassificationTree:
    ...
    def get_solution_vars(self, is_leaf, node_no):
        ...
        if is_leaf:
            ...
            if v == 1:
                applies_split = True
                # find which features the node splits on
                split_on = [self.model.getVarByName('node_{0}_splits_on_feature_{1}'.format(node_no, f)).X for f in
